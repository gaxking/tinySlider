;(function(w){
	...
	function addDotFn() {
		if(this.options.addDot) {
			...
			for(var x in this.options.dots) {
	...
	var TinySlider = function(param) {
		...
		this.Events['touchmove'] = function(e) {
			...
			var y = document.body.scrollTop - moveY<0?0:document.body.scrollTop - moveY;
		...
		this.Events['heartTimer'] = function() {
			...
			}, options.stay + options.duration);
		};

		this.Events['touchend'] = function() {
			dom.style[transName.transition[0]] = "all "+options.duration/1000+"s ease";
